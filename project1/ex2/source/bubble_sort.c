    ...
    while(i>0)
	{
        ...
        for(j=1;j<i;j++)
		{
            if (r[j-1]>r[j])
			{
				...
				r[j]=r[j-1];
				r[j-1]=t;
				flag=0;//发生交换，flag变为0，表明已经有序，之后直接跳出循环。 
                k=j;//将最后一次交换的下标保存，从而减少循环内比较的次数 
            }
        }
        if(flag==1) break;
        i=k;
...
int main()
{
    ...
    sprintf(filename,"D:\\code\\PB14011074-project1\\ex2\\output\\bubble_sort\\result_%d.txt",b);//这里只是为了懒得每一次运行之前改文件代码写的路径而已。。 
    ...
    if((fp=fopen("D:\\code\\PB14011074-project1\\ex2\\output\\bubble_sort\\time.txt", "a"))==NULL)//写入rusult_i.txt中 
	{
        printf("error\n");
        exit(0);
    }
